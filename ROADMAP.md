...
| 2026-08-04 | **`headline` opens a division too**, alongside `section-title` | Which tag the OCR picks is not predictable from the page: this book's grandest chapter openings — an illustration, a circled number, the title in display type — come back as `headline`, and plainer ones as `section-title`. Reading only the second cost eleven divisions. Each missed title was welded onto the passage beneath it and into its hash, and the four back-matter chapters ran together as one division, each counting from ૧ so their ids collided into `v1`…`v1-4` — refs no reader could cite. It also removed the last reason to consider letting a numbering restart open a division: the boundaries were printed all along, and the OCR had found every one |
